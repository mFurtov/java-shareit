...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query("SELECT b FROM Booking b WHERE b.start <= ?1 and b.end >= ?1 and b.booker.id = ?2 order by b.start desc")
    ...
    List<Booking> findByItemIdAndBookerIdAndStatusAndEndBefore(int itemId, int bookerId, BookingStatus status, LocalDateTime dateTime);

    @Query("SELECT b FROM Booking b WHERE b.start <= ?1 AND b.end >= ?1 and b.item.owner.id = ?2 order by b.start desc")
